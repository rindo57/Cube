...
(function() {
  ...
  function enumvar(number, string) {
    Object.defineProperty(this, "number", { value: number, enumerable: true });
    Object.defineProperty(this, "string", { value: string, enumerable: true });
  }
  enumvar.prototype.toString = function() { return this.string; };
  enumvar.prototype.ordinal = function() { return this.number; };
  function makeEnum(a) {
    var o = {};
    ...
    Object.defineProperty(o, "valueOf", {
      value: function(a) { return this[a]; },
      writable: false, configurable: false, enumerable: false
    });
    return o;
  }
  Number.prototype.ordinal = function() { return this + 0; };
  ...
  function verify(s) {
    ...
    var expectedCount = s.length === 24 ? 4 : 9;
    for (var i = 0; i < 6; i++) {
  ...
  function randomCube() {
    ...
    return fc.toString().substring(0, 24); // Truncate to 24 for 2x2
  }
  ...
  function FaceCube(s) {
    this.f = [];
    if (!s) {
      s = this.f.length === 24 ? "UUUURRRRFFFFDDDDLLLLBBBB" : "UUUUUUUUURRRRRRRRRFFFFFFFFFDDDDDDDDDLLLLLLLLLBBBBBBBBB";
    }
    for (var i = 0; i < (s.length || 54); i++) {
  ...
  FaceCube.prototype.toCubieCube = function() {
    ...
    for (var i = 0; i < 8; i++) ccRet.cp[i] = URF;
    for (var i = 0; i < 12; i++) ccRet.ep[i] = UR;
    var col1, col2;
    for (var i in Corner) {
      i = Corner[i];
      for (ori = 0; ori < 3; ori++)
    ...
    if (this.f.length === 54) { // Only process edges for 3x3
      for (var i in Edge) {
        i = Edge[i];
        for (var j in Edge) {
          j = Edge[j];
  ...
  CubieCube.prototype.rotateRight = function(arr, l, r) {
    var temp = arr[r];
    for (var i = r; i > l; i--) arr[i] = arr[i - 1];
    arr[l] = temp;
  };
  CubieCube.prototype.toFaceCube = function() {
    ...
    for (var c in Corner) {
      c = Corner[c];
      var i = c.ordinal();
      var j = this.cp[i].ordinal();
      var ori = this.co[i];
      for (var n = 0; n < 3; n++)
        fcRet.f[cornerFacelet[i][(n + ori) % 3].ordinal()] = cornerColor[j][n];
    }
    if (fcRet.f.length === 54) { // Only process edges for 3x3
      for (var e in Edge) {
        e = Edge[e];
        var i = e.ordinal();
        var j = this.ep[i].ordinal();
        var ori = this.eo[i];
        for (var n = 0; n < 2; n++)
  ...
  CubieCube.prototype.invCubieCube = function(c) {
    ...
    for (var corn in Corner) {
      ...
      if (ori >= 3) c.co[corn.ordinal()] = ori;
      else {
        c.co[corn.ordinal()] = -ori;
        if (c.co[corn.ordinal()] < 0) c.co[corn.ordinal()] += 3;
      }
    }
  };
  CubieCube.prototype.getTwist = function() {
    var ret = 0;
    for (var i = URF.ordinal(); i < DRB.ordinal(); i++) ret = (3 * ret + this.co[i]);
    return ret;
  };
  ...
  CubieCube.prototype.getFlip = function() {
    var ret = 0;
    for (var i = UR.ordinal(); i < BR.ordinal(); i++) ret = (2 * ret + this.eo[i]);
    return ret;
  };
  ...
  CubieCube.prototype.cornerParity = function() {
    ...
    for (var i = DRB.ordinal(); i >= URF.ordinal() + 1; i--)
      for (var j = i - 1; j >= URF.ordinal(); j--)
        if (this.cp[j].ordinal() > this.cp[i].ordinal()) s++;
    return (s % 2);
  };
  CubieCube.prototype.edgeParity = function() {
    ...
    for (var i = BR.ordinal(); i >= UR.ordinal() + 1; i--)
      for (var j = i - 1; j >= UR.ordinal(); j--)
        if (this.ep[j].ordinal() > this.ep[i].ordinal()) s++;
    return (s % 2);
  };
  ...
  CubieCube.prototype.setFRtoBR = function(idx) {
    ...
    for (var e in Edge) {
      e = Edge[e];
      this.ep[e.ordinal()] = DB;
    }
    for (var j = 1, k; j < 4; j++) {
      k = b % (j + 1);
      b /= j + 1;
      b = b | 0;
      while (k-- > 0) this.rotateRight(sliceEdge, 0, j);
    }
    x = 3;
    ...
    x = 0;
    for (var j = UR.ordinal(); j <= BR.ordinal(); j++)
      if (this.ep[j] == DB) this.ep[j] = otherEdge[x++];
  };
  ...
  CubieCube.prototype.setURFtoDLF = function(idx) {
    ...
    for (var c in Corner) {
      c = Corner[c];
      this.cp[c.ordinal()] = DRB;
    }
    for (var j = 1, k; j < 6; j++) {
      k = b % (j + 1);
      b /= j + 1;
      b = b | 0;
      while (k-- > 0) this.rotateRight(corner6, 0, j);
    }
    x = 5;
    ...
    x = 0;
    for (var j = URF.ordinal(); j <= DRB.ordinal(); j++)
      if (this.cp[j] == DRB) this.cp[j] = otherCorner[x++];
  };
  ...
  CubieCube.prototype.setURtoDF = function(idx) {
    ...
    for (var e in Edge) {
      e = Edge[e];
      this.ep[e.ordinal()] = BR;
    }
    for (var j = 1, k; j < 6; j++) {
      k = b % (j + 1);
      b /= j + 1;
      b = b | 0;
      while (k-- > 0) this.rotateRight(edge6, 0, j);
    }
    x = 5;
    ...
    x = 0;
    for (var j = UR.ordinal(); j <= BR.ordinal(); j++)
      if (this.ep[j] == BR) this.ep[j] = otherEdge[x++];
  };
  ...
  CubieCube.prototype.setURtoUL = function(idx) {
    ...
    for (var j = 1, k; j < 3; j++) {
      ...
      b /= j + 1;
      b = b | 0;
      while (k-- > 0) this.rotateRight(edge3, 0, j);
    }
    x = 2;
    for (var j = BR.ordinal(); j >= 0; j--)
      if (a - Cnk(j, x + 1) >= 0) {
  ...
  CubieCube.prototype.getURFtoDLB = function() {
    var perm = [];
    var b = 0;
    for (var i = 0; i < 8; i++) perm[i] = this.cp[i];
    for (var j = 7; j > 0; j--) {
      var k = 0;
  ...
  CubieCube.prototype.setURFtoDLB = function(idx) {
    ...
    for (var j = 1; j < 8; j++) {
      ...
      idx /= j + 1;
      idx = idx | 0;
      while (k-- > 0) this.rotateRight(perm, 0, j);
    }
    var x = 7;
    for (var j = 7; j >= 0; j--) this.cp[j] = perm[x--];
  };
  CubieCube.prototype.getURtoBR = function() {
    var perm = [];
    var b = 0;
    for (var i = 0; i < 12; i++) perm[i] = this.ep[i];
    for (var j = 11; j > 0; j--) {
      var k = 0;
      while (perm[j].ordinal() != j) {
        this.rotateLeft(perm, 0, j);
  ...
  CubieCube.prototype.setURtoBR = function(idx) {
    ...
    var x = 11;
    for (var j = 11; j >= 0; j--) this.ep[j] = perm[x--];
  };
  CubieCube.prototype.verify = function() {
    ...
    for (var i = 0; i < 12; i++)
      if (edgeCount[i] != (this.f.length === 24 ? 0 : 1)) return -2;

    for (var i = 0; i < 12; i++) sum += this.eo[i];
    if (sum % 2 != 0 && this.f.length === 54) return -3;
    ...
    sum = 0;
    for (var i = 0; i < 8; i++) sum += this.co[i];
    if (sum % 3 != 0) return -5;

    if ((this.edgeParity() ^ this.cornerParity()) != 0 && this.f.length === 54) return -6;
  ...
  function generateSlice_URFtoDLF_Parity_Prun(FRtoBR_Move, URFtoDLF_Move) {
    ...
    for (var i = 0; i < (N_SLICE2 * N_URFtoDLF * N_PARITY / 2) | 0; i++) Slice_URFtoDLF_Parity_Prun[i] = -1;
    var depth = 0;
    ...
    while (done != N_SLICE2 * N_URFtoDLF * N_PARITY) {
      for (var i = 0; i < N_SLICE2 * N_URFtoDLF * N_PARITY; i++) {
        ...
        if (CoordCube.getPruning(Slice_URFtoDLF_Parity_Prun, i) == depth) {
          for (var j = 0; j < 18; j++) {
            switch (j) {
              case 3: case 5: case 6: case 8: case 12: case 14: case 15: case 17: continue;
              default:
  ...
  function generateSlice_URtoDF_Parity_Prun(FRtoBR_Move, URtoDF_Move) {
    ...
    for (var i = 0; i < (N_SLICE2 * N_URtoDF * N_PARITY / 2) | 0; i++) Slice_URtoDF_Parity_Prun[i] = -1;
    var depth = 0;
            ...
            switch (j) {
              ...
              default:
                ...
                if (CoordCube.getPruning(Slice_URtoDF_Parity_Prun, (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity) == 0x0f) {
                  CoordCube.setPruning(Slice_URtoDF_Parity_Prun, (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity, (depth + 1));
                  done++;
                }
            }
          }
  ...
  function generateSlice_Twist_Prun(FRtoBR_Move, twistMove) {
    ...
    for (var i = 0; i < (N_SLICE1 * N_TWIST / 2 + 1) | 0; i++) Slice_Twist_Prun[i] = -1;
    var depth = 0;
  ...
  function generateSlice_Flip_Prun(FRtoBR_Move, flipMove) {
    ...
    for (var i = 0; i < (N_SLICE1 * N_FLIP / 2) | 0; i++) Slice_Flip_Prun[i] = -1;
    var depth = 0;
  ...
  function generateTwistMove2x2() {
    var twistMove2x2 = generateArray(N_TWIST_2x2, Int16Array, N_MOVE_2x2);
    var a = new CubieCube();
    for (var i = 0; i < N_TWIST_2x2; i++) {
      a.setTwist(i);
      for (var j = 0; j < 6; j++) {
        for (var k = 0; k < 3; k++) {
          a.cornerMultiply(CubieCube.moveCube[j]);
          twistMove2x2[i][3 * j + k] = a.getTwist();
        }
        a.cornerMultiply(CubieCube.moveCube[j]);
      }
    }
  ...
  function generateCornersMove2x2() {
    var cornersMove2x2 = generateArray(N_CORNERS_2x2, Int16Array, N_MOVE_2x2);
    var a = new CubieCube();
    for (var i = 0; i < N_CORNERS_2x2; i++) {
      a.setURFtoDLB(i);
      for (var j = 0; j < 6; j++) {
        for (var k = 0; k < 3; k++) {
          a.cornerMultiply(CubieCube.moveCube[j]);
          cornersMove2x2[i][3 * j + k] = a.getURFtoDLB();
        }
        a.cornerMultiply(CubieCube.moveCube[j]);
      }
    }
    return cornersMove2x2;
  }

  function generateTwistCornersPrun() {
    var TwistCornersPrun = new Int8Array((N_TWIST_2x2 * N_CORNERS_2x2 / 2) | 0);
    for (var i = 0; i < (N_TWIST_2x2 * N_CORNERS_2x2 / 2) | 0; i++) TwistCornersPrun[i] = -1;
    var depth = 0;
    CoordCube.setPruning(TwistCornersPrun, 0, 0);
    var done = 1;
    while (done != N_TWIST_2x2 * N_CORNERS_2x2) {
      for (var i = 0; i < N_TWIST_2x2 * N_CORNERS_2x2; i++) {
        var twist = (i / N_CORNERS_2x2) | 0;
        var corners = i % N_CORNERS_2x2;
        if (CoordCube.getPruning(TwistCornersPrun, i) == depth) {
          for (var j = 0; j < N_MOVE_2x2; j++) {
            var newTwist = CoordCube2x2.twistMove2x2[twist][j];
            var newCorners = CoordCube2x2.cornersMove2x2[corners][j];
            if (CoordCube.getPruning(TwistCornersPrun, N_CORNERS_2x2 * newTwist + newCorners) == 0x0f) {
              CoordCube.setPruning(TwistCornersPrun, N_CORNERS_2x2 * newTwist + newCorners, depth + 1);
              done++;
            }
          }
        }
    ...
    return TwistCornersPrun;
  }

  function generateTable(table, data) {
    switch (table) {
      case "twistMove": postMessage({type: "", table: ""}); break;
    }
  ...
  function generateTables() {
    ...
    CoordCube2x2.twistMove2x2 = generateTwistMove2x2();
    sendProgressMessage("twistMove2x2");
    CoordCube2x2.cornersMove2x2 = generateCornersMove2x2();
    sendProgressMessage("cornersMove2x2");
    CoordCube2x2.TwistCornersPrun = generateTwistCornersPrun();
    sendProgressMessage("TwistCornersPrun");
  }

  function CoordCube2x2(c) {
    this.twist = c.getTwist();
    this.corners = c.getURFtoDLB();
    this.move = function(m) {
      this.twist = CoordCube2x2.twistMove2x2[this.twist][m];
      this.corners = CoordCube2x2.cornersMove2x2[this.corners][m];
    };
  }
  CoordCube2x2.N_TWIST_2x2 = N_TWIST_2x2;
  CoordCube2x2.N_CORNERS_2x2 = N_CORNERS_2x2;
  CoordCube2x2.N_MOVE_2x2 = N_MOVE_2x2;
  ...
  var Search = {
    ...
    twist2x2: new Int32Array(31),
    corners2x2: new Int32Array(31),

    solutionToString: function(length) {
      var s = "";
    ...
    solution: function(facelets, maxDepth, timeOut, useSeparator) {
      if (facelets.length === 24) return this.solution2x2(facelets, maxDepth, timeOut);
      var count = new Int32Array(6);
      try {
        for (var i = 0; i < 54; i++) count[Color.valueOf(facelets.charAt(i)).ordinal()]++;
      } catch (e) {
        return "Error 1";
      }
      for (var i = 0; i < 6; i++) if (count[i] != 9) return "Error 1";
      var fc = new FaceCube(facelets);
      var cc = fc.toCubieCube();
      if ((s = cc.verify()) != 0) return "Error " + Math.abs(s);
      ...
      do {
        do {
          if ((depthPhase1 - n > this.minDistPhase1[n + 1]) && !busy) {
            ...
            do {
              if (++this.ax[n] > 5) {
                if ((new Date()).getTime() - tStart > timeOut << 10) return "Error 8";
                if (n == 0) {
        ...
        this.minDistPhase1[n + 1] = Math.max(CoordCube.getPruning(CoordCube.Slice_Flip_Prun, CoordCube.N_SLICE1 * this.flip[n + 1] + this.slice[n + 1]),
                                          CoordCube.getPruning(CoordCube.Slice_Twist_Prun, CoordCube.N_SLICE1 * this.twist[n + 1] + this.slice[n + 1]));
        if (this.minDistPhase1[n + 1] == 0 && n >= depthPhase1 - 5) {
          ...
          if (n == depthPhase1 - 1 && (s = this.totalDepth(depthPhase1, maxDepth)) >= 0) {
            if (s == depthPhase1 || (this.ax[depthPhase1 - 1] != this.ax[depthPhase1] && this.ax[depthPhase1 - 1] != this.ax[depthPhase1] + 3))
    ...
    totalDepth: function(depthPhase1, maxDepth) {
      ...
      if ((d1 = CoordCube.getPruning(CoordCube.Slice_URFtoDLF_Parity_Prun,
          (CoordCube.N_SLICE2 * this.URFtoDLF[depthPhase1] + this.FRtoBR[depthPhase1]) * 2 + this.parity[depthPhase1])) > maxDepthPhase2) return -1;
      for (var i = 0; i < depthPhase1; i++) {
        mv = 3 * this.ax[i] + this.po[i] - 1;
      ...
      if ((d2 = CoordCube.getPruning(CoordCube.Slice_URtoDF_Parity_Prun,
          (CoordCube.N_SLICE2 * this.URtoDF[depthPhase1] + this.FRtoBR[depthPhase1]) * 2 + this.parity[depthPhase1])) > maxDepthPhase2) return -1;
      if ((this.minDistPhase2[depthPhase1] = Math.max(d1, d2)) == 0) return depthPhase1;
      var depthPhase2 = 1;
      var n = depthPhase1;
      var busy = false;
      ...
      do {
        ...
        this.minDistPhase2[n + 1] = Math.max(CoordCube.getPruning(CoordCube.Slice_URtoDF_Parity_Prun, (CoordCube.N_SLICE2 * this.URtoDF[n + 1] + this.FRtoBR[n + 1]) * 2 + this.parity[n + 1]),
                                          CoordCube.getPruning(CoordCube.Slice_URFtoDLF_Parity_Prun, (CoordCube.N_SLICE2 * this.URFtoDLF[n + 1] + this.FRtoBR[n + 1]) * 2 + this.parity[n + 1]));
      } while (this.minDistPhase2[n + 1] != 0);
      return depthPhase1 + depthPhase2;
    },
    solution2x2: function(facelets, maxDepth, timeOut) {
      var count = new Int32Array(6);
      try {
        for (var i = 0; i < 24; i++) count[Color.valueOf(facelets.charAt(i)).ordinal()]++;
      } catch (e) {
        return "Error 1";
      }
      for (var i = 0; i < 6; i++) if (count[i] != 4) return "Error 1";
      var fc = new FaceCube(facelets);
      var cc = fc.toCubieCube();
      if ((s = cc.verify()) != 0) return "Error " + Math.abs(s);
      var c = new CoordCube2x2(cc);
      this.ax[0] = 0;
      this.po[0] = 0;
      this.twist2x2[0] = c.twist;
      this.corners2x2[0] = c.corners;
      this.minDistPhase1[1] = 1;
      var mv = 0, n = 0;
      var busy = false;
      var depth = 1;
      ...
      do {
        do {
          if ((depth - n > this.minDistPhase1[n + 1]) && !busy) {
            if (this.ax[n] == 0 || this.ax[n] == 3) this.ax[++n] = 1;
            else this.ax[++n] = 0;
            this.po[n] = 1;
          } else if (++this.po[n] > 3) {
            do {
              if (++this.ax[n] > 5) {
                if (n == 0) {
                  if (depth >= maxDepth) return "Error 7";
                  depth++;
                  this.ax[n] = 0;
                  this.po[n] = 1;
                  busy = false;
        ...
        mv = 3 * this.ax[n] + this.po[n] - 1;
        this.twist2x2[n + 1] = CoordCube2x2.twistMove2x2[this.twist2x2[n]][mv];
        this.corners2x2[n + 1] = CoordCube2x2.cornersMove2x2[this.corners2x2[n]][mv];
        this.minDistPhase1[n + 1] = CoordCube.getPruning(CoordCube2x2.TwistCornersPrun, CoordCube2x2.N_CORNERS_2x2 * this.twist2x2[n + 1] + this.corners2x2[n + 1]);

        if (this.minDistPhase1[n + 1] == 0 && n >= depth - 1) {
          return this.solutionToString(n + 1);
        }
      } while ((new Date()).getTime() - tStart < timeOut * 1000);
      return "Error 8";
    }
  };
})();
